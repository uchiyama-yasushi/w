...
uint8_t test_tls[THREAD_STACK_SIZE (THREAD_STACKSIZE_DEFAULT)];
thread_t test_th;
...
board_main (uint32_t arg __attribute__((unused)))
{
  ...
  th = thread_create_with_stack (test_tls, THREAD_STACKSIZE_DEFAULT, "test", test_thread,
		      0x1234abcd);
  thread_start (th);
